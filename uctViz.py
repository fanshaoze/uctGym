...
class TreeGraph(object):
	...
	def drawSearch(self, uctTree, _destLeafIndex, fileName):
		...
		self.grap_g.render(fileName, view=True)
	...
	def drawTree(self, currentIndex, v_stateNode, parentsIndex, parentState, actionid):
		actVisitCounter = 0
		...
		if parentState is not None:
			...
			lineLable = self.getLineLable(parentState, actionid)
		...
		for i in range(0, actSize):
			actionId = v_stateNode.actVect_[i].id
			# print (actionId)
			...
			for j in range(0, stateSize):
				...
				self.drawTree(childIndex, v_childStateNode, currentIndex, v_stateNode, actionId)
